...
export default function AdminPanel() {
  ...
  const handleValidateLink = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refresh the invite data to show updated status
        await fetchInvite();
      } else {
        const error = await response.json();
  ...
  const getStatusText = (invite: InviteData) => {
    ...
    const daysLeft = Math.max(0, 30 - Math.floor(diffInDays));

    if (daysLeft === 0) return 'Expired';
    return `Active (${daysLeft} days left)`;
  };

  const getStatusColor = (invite: InviteData) => {
    if (!invite.isActive) return 'text-red-500';
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <span className={`font-semibold ${getStatusColor(invite)}`}>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-md">
            <p className="text-sm text-blue-800">
              <strong>Note:</strong> The new link will replace the current one and be active for 30 days OR 400 invitations.
